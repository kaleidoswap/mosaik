...
EOF
    ...
    fund_wallet
    # Create the user + market-maker wallets after funding so fund_wallet's
    # implicit wallet calls (getnewaddress, sendtoaddress) resolve to mosaik
    # unambiguously. The UI server also ensures these on startup.
    for w in mosaik-user mosaik-mm-a mosaik-mm-b mosaik-mm-c; do
        cli createwallet "$w" >/dev/null 2>&1 \
            || cli loadwallet "$w" >/dev/null 2>&1 || true
    done
